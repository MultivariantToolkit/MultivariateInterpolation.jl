...
@inline function _assert_valid_dimension(dim)
    dim >0 || throw(
        ArgumentError(
            "Dimension needs to be a positive integer! <$dim> given."
        )
    )    
end
...
@inline function _assert_valid_lp(lp)
    lp > zero(lp) || throw(
        ArgumentError(
            "The lp-degree needs to be a positive integer! <$lp> given."
        )
    )
end
...
function MultiIndexSet(dim::T, deg::T, lp::Tlp) where {T<:Integer, Tlp<: Real}
	...
	MultiIndexSet(_generate_exponents_static(dim,deg,lp))
end
